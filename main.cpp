...
int main( int argc, char** argv ) {
    ...
    for( vector<string>::const_iterator itr = highlighter->getHighlightGroups().begin() ;
        itr != highlighter->getHighlightGroups().end() ; ++ itr ) {
        /* Iterate through all the highlighting
         * groups and print out the vim command
         * to add that highlight */
        const vector<string>* vec = highlighter->getHighlightsForGroup( *itr );
        if( vec != NULL ) {
